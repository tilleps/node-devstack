...
function swaggerToPopulate(value) {
  ...
  var items = value.split(',').map(function (item) {
    ...
    return { path: parts[0], select: parts[1] };
  });
  ...
  return function (req, res, next) {
    ...
    function validateSuccess(values) {
      ...
      model.validate(function (err) {
        ...
        //
        Model.findByIdAndUpdate(req.swagger.params.id.value, { '$set': flattened }, { new: true }, function (err, doc) {
...
module.exports.add = function (Model) {
  return function (req, res, next) {
    ...
    */
    ...
    validate.isValid(req.body, constraints, function (values) {
...
module.exports.browse = function (Model) {
  return function (req, res, next) {
    ...
    var where = createWhere(req.query._where);
